...
public class GameEscorteeManager : MonoBehaviour
{
    ...
    {
        // TODO: Spawn point for escortee
        SpawnEscortee(gameManager.EscorteePrefab.transform);
    }
    public void SpawnEscortee(Transform spawnPoint)
    {
        if (!gameManager.ActiveEscortee)
        {
            gameManager.ActiveEscortee = Instantiate(gameManager.EscorteePrefab, spawnPoint.position, Quaternion.identity);
        }
        else
        {
